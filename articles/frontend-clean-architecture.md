...
とあるシステムのリプレイス。フロントエンド担当は一人。

デザインは別チームで作っていただけたため、ライブラリなどの技術選定、フィジビリティチェック、設計、実装——すべて一人でこなした。最終的に 6 万行規模の Vue プロジェクトになった。

プロジェクトの状況整理と情報収集を行った結果、**堅牢で壊れづらく、変更に強い設計**が必要だと判断した。要件変更が頻繁に入ることは容易に想像できたし、デザイナーがデザインを保守しやすい構造にもしたかった。

そこで Clean Architecture の原則を参考にすることにした。ただし、Clean Architecture はあくまでオブジェクト指向の世界で生まれた概念であり、フロントエンドにそのまま適用できないことは理解していた。フロントエンドの特性に合わせて、どのような形に落とし込むか——それがこのプロジェクトの設計上の挑戦だった。

試行錯誤の末にたどり着いたのが、この責任分離のレイヤードアーキテクチャである。
